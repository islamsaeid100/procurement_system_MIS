...
class PurchaseOrderSerializer(serializers.ModelSerializer):
    items = OrderItemSerializer(many=True) # شيلنا read_only عشان نقدر نبعتهم في الـ POST
    ...
    class Meta:
        ...
        # حماية الحقول التي يولدها السيرفر تلقائياً
    ...
    def create(self, validated_data):
        ...
        total = 0
        # إنشاء العناصر المرتبطة وحساب الإجمالي
        for item_data in items_data:
            product = item_data['product']
            quantity = item_data['quantity']
            price = product.unit_price
            OrderItem.objects.create(order=order, product=product, quantity=quantity, unit_price=price)
